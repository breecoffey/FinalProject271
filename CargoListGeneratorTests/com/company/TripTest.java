...
public class TripTest {
    ...
    /**
     * Tests that the optimal list is empty when no items fit and the list of items is largee
     * @throws Exception if test fails
     * @result the expected cargo item list is empty
     */
    ...
    /**
     * Tests that the optimal list is stored when two identical items exist on the list and there are many items on the list
     * @throws Exception if test fails
     * @result the expected cargo item list holds one or the other
     */
    ...
    /**
     * Tests to see if the proper solution is found whn there should be only one item taken along from list of potential items.
     * When two identical items on list of potential items. neither item is in the solution
     * @throws Exception if test fails
     * @result the expected cargo item list holds both items or neither item in each assertEquals
     */
    ...
    /**
     * //Tests to see if the proper solution is found when there should be more than one item taken along from list of potential.
     * One possible solution when two identical items on list of potential items. neither item is in the solution
     * @throws Exception
     */
    @Test
    ...
    /**
     * //Tests to see if at least one proper solution is found when:
     * there are a large amount of items and a few are below the weight limit
     * @throws Exception when fails
     */

    @Test
    public void testForLargeAmountOfItems() throws Exception {
        Scanner sc = null;
        try {
            sc = new Scanner(new File("testfile4.txt"));
        ...
        while (sc.hasNextLine()) {
            ...
            breeList.addCargoItemToList(cargoItem);

            if (sc.hasNextLine()) {
                sc.nextLine();
            }
        }
        Plane aBigPlane = new Plane("A Big Plane", 15);
        Trip homeTrip = new Trip(aBigPlane);
        homeTrip.loadAirplane(breeList);

        assertEquals("[Item: Item 60, Weight: 3, Value: $50\n" +
                ", Item: Item 57, Weight: 2, Value: $87\n" +
                ", Item: Item 53, Weight: 4, Value: $85\n" +
                ", Item: Item 48, Weight: 5, Value: $200\n" +
                "]",homeTrip.getPlaneForTrip().getItemsToTake().getItemArrayList().toString());
